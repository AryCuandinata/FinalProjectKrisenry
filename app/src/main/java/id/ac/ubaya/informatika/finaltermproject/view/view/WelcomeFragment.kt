...
class WelcomeFragment : Fragment(){
    ...
    override fun onStartJourneyClick(v: View) {
        val radio = v.findViewById<RadioButton>(radioGroup.checkedRadioButtonId)
        val radioGender = v.findViewById<RadioButton>(radioGroupGenderWellcome.checkedRadioButtonId)
        val model = User(textInputName.text.toString(), textInputAge.text.toString().toInt(), radio.tag.toString().toInt(), textInputWeight.text.toString().toInt(), textInputHeight.text.toString().toInt(), radioGender.toString().toInt())
        val list = listOf(model)
        viewModel.insertUser(list)
